...
- `/src/onboard/shared/schemas.py` (Step 1)
- `/src/onboard/layer_02_sensor/schema.py`, `mock_source.py` (Step 2 — imagery/acoustic 필드에 심어둔 mock 라벨 힌트)
- `/src/onboard/layer_03_abstraction/run.py` 및 채널 모듈 전체 (Step 3 — 이 step은 이 오케스트레이터에 3채널을 추가한다)
...
03의 AI 사용 채널 3종을 stub으로 추가한다. 실제 모델 로딩은 하지 않고 `src/onboard/ai_stubs/`의 함수가 raw envelope의 mock 라벨 힌트를 읽어 고정 결과를 리턴한다.

### 1) AI stub 모듈 `src/onboard/ai_stubs/`
...
`src/onboard/layer_03_abstraction/`에 추가:
...
`src/onboard/layer_03_abstraction/run.py`가 이제 11채널을 모두 호출한다.
