...
## 아키텍처 규칙
- CRITICAL: 각 레이어는 `d4d_pipeline/{layer_name}/` 패키지로 분리. 레이어 간 통신은 오직 JSON-직렬화 가능한 dict로만. 레이어가 다른 레이어의 내부 모듈을 직접 import 금지.
...
- CRITICAL: 05 Risk Assessment의 `RAC_MATRIX`(6×4)와 04 Threat Modeling의 `PHASE_THREAT_MULTIPLIER`, `SIGNAL_TO_THREAT` 매핑은 코드에 하드코딩 상수로 두고 함수 인자로 오버라이드 불가.
...
python3 -m d4d_pipeline.run examples/scenario_t3.json  # 종단 실행
```
