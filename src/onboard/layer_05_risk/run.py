...
오버라이드 불가 (ADR-003 / SCC-1). candidates 가 비면 ambient_rac 는 항상 "Low"
(issue #24 Lead 결정, exposure 기반 Medium 승격 폐기).
...
from ..shared.constants import SEVERITY_ORDER
...
def run(
    ...
    *,
    ...
    if not candidates_in:
        # issue #24 Lead 결정: 위협 후보가 없으면 ambient_rac는 항상 Low
        # (exposure≥0.7→Medium 승격 규칙 폐기, background_exposure_score는 참고 지표로만 소비).
