...
## 성공 기준
- `raw_t3.json`+`mission_brief_t3.json`(근접 소화기, 정찰) 입력 → 06이 `flight_action=ALTITUDE_CHANGE`, RAC `Serious`, `payload_action=[]` 를 산출한다 (정찰 base_rate → Serious 등급, layer 05). 실행: `python3 -m onboard examples/raw_t3.json examples/mission_brief_t3.json`.
- 고위협(High) 대응 경로는 `mission_brief_strike.json`(타격 컨텍스트)으로 검증: `flight_action=RTL`, `payload_action=[DATA_WIPE, WEAPON_DROP]` 산출.
- 6개 레이어 각각에 대해 D4D 문서의 예시 JSON을 골든 케이스로 통과 (`examples/expected_*.json`, `tests/integration/test_e2e_golden.py`).
- 결정론적 경로와 AI 강화판이 각각 계산되고, 교차검증 결과(`ai_reliability`)가 통보 페이로드에 포함된다.

## 디자인
- UI 없음. CLI로 JSON 입력받아 JSON 출력.
- 로그는 사이클별 각 레이어 출력을 JSON Lines로 append (CLI `--log <path>` 옵션, 레이어당 1줄 `{seq, layer, output}`). 오케스트레이터는 순수 유지 — 로깅은 CLI(유즈사이트) 책임.
