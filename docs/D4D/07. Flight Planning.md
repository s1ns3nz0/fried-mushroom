...
> ⚠️ 참고: `route.py`(`generate_route()`)가 산출하는 실제 웨이포인트 목록은 `flight_action` 기준(REROUTE 등에서도 corridor 원래 좌표 + 고도델타만 반영)이라, `target_bearing_deg`(위협 반대방향)가 아직 수평 경로 자체에는 반영되지 않습니다. 즉 "회피"가 실제로 옆으로 비키는 궤적까지는 이번 라운드 범위 밖입니다 — `reroute_anchor="mission_corridor_resume"`은 이 갭과 별개로, "회피 국면이 끝났을 때 오토파일럿에게 뭘 지시할지"만 명확히 합니다.
...
| route | `route.py`(`generate_route()`)가 산출하는 terrain-aware 웨이포인트 목록(`[{lat, lon, alt_m, clearance_m}, ...]`) — 별도 기능(#102/#120), 아래 참고 |
...
> ⚠️ 참고: `route.py`가 산출하는 실제 웨이포인트 목록은 `flight_action` 기준(REROUTE 등에서도 corridor 원래 좌표 + 고도델타만 반영)이라, `target_bearing_deg`(위협 반대방향)가 아직 수평 경로 자체에는 반영되지 않습니다. 즉 "회피"가 실제로 옆으로 비키는 궤적까지는 이번 라운드 범위 밖입니다 — `reroute_anchor="mission_corridor_resume"`은 이 갭과 별개로, "회피 국면이 끝났을 때 오토파일럿에게 뭘 지시할지"만 명확히 합니다.
