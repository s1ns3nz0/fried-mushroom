...
| route | `route.py`(`generate_route()`)가 산출하는 terrain-aware + 회피 수평궤적이 반영된 웨이포인트 목록(`[{lat, lon, alt_m, terrain_elev_m, clearance_m}, ...]`) — 아래 "수평 회피 궤적" 참고. **실 DEM(#341):** `terrain_elev_m` = 결정론 heightmap(`terrain.py`, vizsim/app.js PEAKS 포트) 표고, `clearance_m = alt_m - terrain_elev_m`(봉우리 교차/저고도 시 급감·음수 → 지형충돌 위험 실값). flat DEM=0 stub 대체 |
