...
| MAINTAIN | NONE | 경로 변경 없음 — reroute_anchor=null |
...
| reroute_anchor | 방향/재계획 기준(`threat_reverse(channel)` / `terrain_fallback` / `last_known_good_position` / `optimal_terrain` / `altitude_only`) — replan_scope=NONE(MAINTAIN) 시 null |
