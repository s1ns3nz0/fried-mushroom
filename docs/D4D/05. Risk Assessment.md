...
`04`가 이미 "여러 위협이 동시에 뜰 수 있다"는 전제로 `candidates[]`를 설계했으므로, `05`는 `primary` 하나만 보지 않고 candidates 전체를 각각 독립적으로 L×S→RAC 계산합니다. 위협이 하나도 매칭되지 않았을 때는 `04`가 전달하는 `background_exposure_score`(T6, 배경 노출도)만 참고해서 최소 RAC 하한을 둡니다 — exposure_score≥0.7이면 구체적 위협이 없어도 RAC를 `Medium`으로, 그 밑이면 `Low`로 둡니다.
...
| 필드 | 의미 |
|---|---|
| candidates[] | priority_rank로 정렬된 위험평가 결과 배열 |
...
| ambient_rac | candidates가 비었을 때만 값이 있음(background_exposure_score 기반 최소 하한) |
...
| l_value_to_class 임계값 | 0.5/0.3/0.15/0.05 | MIL-STD-882E Table II 형식 차용 |
...
| ambient_rac 배경노출 임계값 | 0.7 | 팀 설정값 |
